...
<body>
    <div class="comment_wrapper">
		...
		if (isset($_SESSION['username'])) {
			...
			$user_details_query = mysqli_query($con, "SELECT * FROM lecturer WHERE username='$userLoggedIn'");
		...
		if (isset($_POST['postComment' . $post_id])) {
			...
			$notified_lecturer = array();
			while($row = mysqli_fetch_array($get_commenters)) {
	
				if($row['posted_by'] != $posted_to && $row['posted_by'] != $user_to 
					&& $row['posted_by'] != $userLoggedIn && !in_array($row['posted_by'], $notified_lecturer)) {
	
					$notification = new User2($con, $userLoggedIn);
					$notification->insertNotification($post_id, $row['posted_by'], "comment_non_owner");
	
					array_push($notified_lecturer, $row['posted_by']);
				}
