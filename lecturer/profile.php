...
if (isset($_GET['profile_username'])) {
    ...
    $user_details_query = mysqli_query($con, "SELECT * FROM lecturer WHERE username='$username'");
...
if (isset($_POST['profile-updateBtn'])) {
    ...
    $bio = $_POST['bio'];
    $query = mysqli_query($con, "UPDATE lecturer SET first_name ='$firstName' WHERE username LIKE '$username'");
    $query1 = mysqli_query($con, "UPDATE lecturer SET last_name ='$lastName' WHERE username LIKE '$username'");
    $query2 = mysqli_query($con, "UPDATE lecturer SET phoneNumber ='$phoneNumber' WHERE username LIKE '$username'");
    $query3 = mysqli_query($con, "UPDATE lecturer SET bio  ='$bio' WHERE username LIKE '$username'");
    header("Location: $username");
}

$teaching  = new User($con,$code ,$username);
$query = mysqli_query($con, "SELECT * FROM lecturer WHERE username='$username'");
...
<main class="profile-body">
    ...
    <section class="side-bar">
        <div class="profile__image"><button onclick="history.go(-1);">Back</button><br><br>
