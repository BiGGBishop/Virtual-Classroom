	...
	class Post
	{
		...
		public function submitPost($body, $fileName, $fileDestination,$user_to)
		{
			...
			if ($check_empty != "" && $fileName == "") {
				...
				//insert post 
				$query = mysqli_query($this->con, "INSERT INTO posts VALUES('', '$check_empty', '$added_by','$this->code', '$user_to', '$date_added', 'no', 'no','','','student')");
			...
			if ($fileName != "") {  //only assignment
				...
				//insert post
				$query = mysqli_query($this->con, "INSERT  INTO posts VALUES('', '$body', '$added_by','$this->code', '$user_to', '$date_added', 'no', 'no','$fileName','$fileDestination','student')");
				$returned_id = mysqli_insert_id($this->con);
			}
		}
			...
			if (mysqli_num_rows($data_query) > 0) {
				...
				while ($row = mysqli_fetch_array($data_query)) {
					...
					if ($user_logged_obj->isStudent($added_by)) {
						...
						$user_details_query = mysqli_query($this->con, "SELECT first_name, last_name, profilePic FROM lecturer WHERE username='$added_by'");
...
public function loadFiles()
{
	...
	$str = ""; //String to return
	$data_query = mysqli_query($this->con, "SELECT * FROM posts WHERE courseCode='$this->code' AND usertype ='student' ORDER BY id DESC");
...
public function loadFiles2()
{
	...
	$str = ""; //String to return
	$data_query = mysqli_query($this->con, "SELECT * FROM posts WHERE courseCode='$this->code' AND usertype ='lecturer' ORDER BY id DESC");
